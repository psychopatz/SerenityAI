    ...
    public ResponseEntity<?> getProfile(@RequestParam String email) {
        try {
            UserEntity user = userService.getUserByEmail(email); // Retrieve the user by email
            return new ResponseEntity<>(user, HttpStatus.OK);
    ...
    @PutMapping("/profile")
public ResponseEntity<?> updateProfile(@RequestBody UserEntity newUserDetails) {
    try {
        UserEntity updatedUser = userService.updateUserByEmail(newUserDetails.getEmail(), newUserDetails);
        return new ResponseEntity<>(updatedUser, HttpStatus.OK);
    } catch (NameNotFoundException e) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of("message", "User not found"));
    }
